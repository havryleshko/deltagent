...
def _round3_dir() -> Path:
    return Path(__file__).resolve().parent / "round3"
...
def _run_single_bundle(bundle: dict[str, Any], round3_dir: Path) -> Round3Entry:
    slug = str(bundle["slug"])
    workbook_path = round3_dir / str(bundle["xlsx"])
    mock_context_path = round3_dir / str(bundle["mock_context"])
    oracle_path = round3_dir / str(bundle["oracle"])
...
def _build_mapping(entries: list[Round3Entry]) -> list[dict[str, str]]:
    return [
        {
            "slug": entry.slug,
...
def run_round3_score_only(
    *,
    mapping_path: Path,
    report_stem: str,
    header_note: str = "",
) -> tuple[Path, Path]:
    round3_dir = _round3_dir()
    root = _repo_root()
    ...
    out_mapping = round3_dir / f"{report_stem}_mapping.json"
    ...
    report_title = (
        "Round 3 Raw Baseline" if "raw" in report_stem else "Round 3 Post-fix Baseline"
    )
    report_path = round3_dir / f"{report_stem}.md"
...
def run_round3(*, report_stem: str) -> tuple[Path, Path]:
    round3_dir = _round3_dir()
    bundles = _load_manifest(round3_dir / "manifest.json")
    entries = [_run_single_bundle(bundle, round3_dir) for bundle in bundles]
    mapping = _build_mapping(entries)

    mapping_path = round3_dir / f"{report_stem}_mapping.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run round3 bundles and score outputs.")
    parser.add_argument(
        "--phase",
        choices=["raw", "postfix"],
        default="raw",
    ...
    stem = "round3_raw_baseline" if args.phase == "raw" else "round3_postfix_baseline"
    if args.score_only_from:
        note = ""
        if args.phase == "postfix":
            note = (
                "> **Postfix LLM re-run pending:** Anthropic API returned insufficient credits during "
                "the postfix agent pass. This report re-scores the saved runs from the mapping file "
                "(eval harness + mock lookup fixes; run JSONs predate the latest `agent/prompts.py` "
                "edit). After restoring API access, run "
                "`PYTHONPATH=<repo> python3 evals/run_round3.py --phase postfix` for a true post-fix "
                "LLM baseline.\n"
            )
        mapping_path, report_path = run_round3_score_only(
            mapping_path=args.score_only_from,
            report_stem=stem,
            header_note=note,
        )
    else:
        mapping_path, report_path = run_round3(report_stem=stem)
